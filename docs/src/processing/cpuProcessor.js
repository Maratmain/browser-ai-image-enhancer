...
export async function processWithCpu(source, parameters, tileSize, hooks) {
    ...
    for (const tile of tiles) {
        ...
        for (let offset = 0; offset < image.data.length; offset += 4) {
            ...
            if (alpha <= 0)
                continue;
            const [red, green, blue] = correctRgb((image.data[offset] ?? 0) / 255, (image.data[offset + 1] ?? 0) / 255, (image.data[offset + 2] ?? 0) / 255, parameters);
