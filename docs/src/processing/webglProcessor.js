...
function compileShader(gl, type, source) {
    ...
    if (shader === null)
        throw new EnhancerError("WEBGL_INITIALIZATION_FAILED", "Unable to create WebGL shader");
...
function createProgram(gl) {
    ...
    if (program === null)
        throw new EnhancerError("WEBGL_INITIALIZATION_FAILED", "Unable to create WebGL program");
...
export function queryWebGlLimits() {
...
export async function processWithWebGl(source, parameters, requestedTileSize, hooks) {
    ...
    const tileSize = Math.max(64, Math.min(requestedTileSize, limits?.maxTextureSize ?? requestedTileSize, limits?.maxRenderbufferSize ?? requestedTileSize, limits?.maxViewportWidth ?? requestedTileSize, limits?.maxViewportHeight ?? requestedTileSize));
