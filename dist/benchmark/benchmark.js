import { ImageEnhancer } from "../src/index.js";
function element(id) {
...
const backendSelect = element("backend");
const progress = element("benchmark-progress");
const status = element("benchmark-status");
const percent = element("benchmark-percent");
const bar = element("benchmark-bar");
const results = element("results");
const summary = element("summary");
const errorOutput = element("benchmark-error");
let latestReport;
    ...
    for (const row of rows) {
        ...
        values.forEach((value, index) => {
            ...
            if (index === values.length - 1)
                td.className = row.success ? "pass-text" : "fail-text";
            tr.append(td);
        });
        results.append(tr);
    }
}
function summaryCard(label, value, passed) {
...
async function runBenchmark() {
    ...
    try {
        ...
        const warmId = enhancer.createTask(warm, { outputType: "image/jpeg", processingBackend: backendSelect.value });
        ...
        for (let index = 0; index < cases.length; index += 1) {
            ...
            const taskId = enhancer.createTask(blob, {
                ...
                processingBackend: backendSelect.value
            });
            try {
        ...
        summary.replaceChildren(summaryCard("Среднее", `${(mean / 1000).toFixed(2)} с`, mean <= 5000), summaryCard("P95", `${(p95 / 1000).toFixed(2)} с`, p95 <= 15000), summaryCard("Максимум", `${(maximum / 1000).toFixed(2)} с`, maximum <= 30000));
