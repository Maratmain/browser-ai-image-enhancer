import { ImageEnhancer, EnhancerError } from "../src/index.js";
function element(id) {
...
const fileCard = element("file-card");
const fileName = element("file-name");
const fileMeta = element("file-meta");
const removeButton = element("remove-button");
const enhanceButton = element("enhance-button");
const cancelButton = element("cancel-button");
const outputType = element("output-type");
const processingBackend = element("processing-backend");
const progressBlock = element("progress-block");
const progressBar = element("progress-bar");
const progressValue = element("progress-value");
const statusLabel = element("status-label");
const errorBox = element("error-box");
const warningBox = element("warning-box");
const emptyPreview = element("empty-preview");
const comparison = element("comparison");
const beforeImage = element("before-image");
const afterImage = element("after-image");
const beforeLayer = element("before-layer");
const comparisonDivider = element("comparison-divider");
const comparisonRange = element("comparison-range");
const resultSection = element("result-section");
const downloadButton = element("download-button");
const capabilitiesGrid = element("capabilities-grid");
const diagnosticsButton = element("diagnostics-button");
const diagnosticsOutput = element("diagnostics-output");
...
function capabilityCard(label, passed, value) {
...
function renderCapabilities(capabilities) {
    capabilitiesGrid.replaceChildren(capabilityCard("Web Worker", capabilities.worker), capabilityCard("OffscreenCanvas", capabilities.offscreenCanvas), capabilityCard("WebGL2", capabilities.webgl2, capabilities.webgl2 ? `PASS · ${capabilities.maxTextureSize ?? "?"} px` : "FALLBACK"), capabilityCard("WebAssembly ML", capabilities.wasm, capabilities.inferenceBackend.toUpperCase()), capabilityCard("JPEG encoder", capabilities.jpegEncode), capabilityCard("PNG encoder", capabilities.pngEncode));
}
async function processSelected() {
    if (selectedFile === undefined || currentTaskId !== undefined)
        return;
    errorBox.classList.add("hidden");
    warningBox.classList.add("hidden");
...
cancelButton.addEventListener("click", () => {
    if (currentTaskId !== undefined)
        void enhancer.abortTask(currentTaskId);
});
downloadButton.addEventListener("click", () => {
    if (resultBlob === undefined || resultUrl === undefined)
        return;
...
diagnosticsButton.addEventListener("click", async () => {
    ...
    try {
        ...
        const blob = await new Promise((resolve, reject) => canvas.toBlob((value) => value === null ? reject(new Error("PNG encode failed")) : resolve(value), "image/png"));
        const runs = [];
