...
function conv3x3Stride2(input, inputSize, inputChannels, outputChannels, weights, bias) {
...
function depthwisePointwise(input, inputSize, inputChannels, outputChannels, depthwiseWeights, depthwiseBias, pointwiseWeights, pointwiseBias) {
    const outputSize = Math.ceil(inputSize / 2);
...
export function inferWithJavaScript(input) {
    const b1 = conv3x3Stride2(input, 64, 3, 8, MODEL_WEIGHTS.conv1W, MODEL_WEIGHTS.conv1B);
    const b2 = depthwisePointwise(b1, 32, 8, 16, MODEL_WEIGHTS.dw2W, MODEL_WEIGHTS.dw2B, MODEL_WEIGHTS.pw2W, MODEL_WEIGHTS.pw2B);
    const b3 = depthwisePointwise(b2, 16, 16, 24, MODEL_WEIGHTS.dw3W, MODEL_WEIGHTS.dw3B, MODEL_WEIGHTS.pw3W, MODEL_WEIGHTS.pw3B);
    const b4 = depthwisePointwise(b3, 8, 24, 32, MODEL_WEIGHTS.dw4W, MODEL_WEIGHTS.dw4B, MODEL_WEIGHTS.pw4W, MODEL_WEIGHTS.pw4B);
