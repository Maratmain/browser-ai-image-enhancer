...
const DEFAULT_HEIC_DECODER_URL =
  "https://cdn.jsdelivr.net/npm/heic-to@1.5.2/dist/next/heic-to.js";

function resolveEnhancerOptions(options: ImageEnhancerOptions = {}): ResolvedEnhancerOptions {
  const defaults = {
...
function resolveTaskOptions(options: EnhanceTaskOptions = {}): ResolvedTaskOptions {
...
export class ImageEnhancer extends EventTarget {
  ...
  #readyPromise?: Promise<EnhancerCapabilities>;
  #disposed = false;
  ...
  ready(): Promise<EnhancerCapabilities> {
    this.#assertActive();
  ...
  createTask(source: File | Blob, options: EnhanceTaskOptions = {}): string {
    this.#assertActive();
  ...
  getStatus(taskId: string): TaskInfo {
    this.#assertActive();
    return this.#tasks.get(taskId);
  }

  async abortTask(taskId: string): Promise<AbortResult> {
    this.#assertActive();
    const current = this.#tasks.get(taskId);
  ...
  #dispatchStatus(task: TaskInfo): void {
