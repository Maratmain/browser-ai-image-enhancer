import type { OutputMimeType, ResolvedTaskOptions, TaskWarning } from "../api/types.js";
...
function resolveOutputType(
  requested: ResolvedTaskOptions["outputType"],
  inputFormat: string,
  hasAlpha: boolean
): OutputMimeType {
  if (requested !== "auto") return requested;
  if (hasAlpha || inputFormat === "png" || inputFormat === "bmp") return "image/png";
  return "image/jpeg";
}
...
export async function encodeCanvas(
  ...
  const actualType: OutputMimeType = blob.type === "image/jpeg" ? "image/jpeg" : "image/png";
