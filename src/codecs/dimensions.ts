import type { InputFormat } from "../api/types.js";
...
function probePng(bytes: Uint8Array): ImageProbe {
  if (bytes.length < 29) throw new EnhancerError("INVALID_IMAGE", "PNG header is truncated", { stage: "validating" });
...
function probeBmp(bytes: Uint8Array): ImageProbe {
  if (bytes.length < 30) throw new EnhancerError("INVALID_IMAGE", "BMP header is truncated", { stage: "validating" });
  const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
...
function probeJpeg(bytes: Uint8Array): ImageProbe {
  let offset = 2;
...
export async function probeImage(blob: Blob, format: InputFormat): Promise<ImageProbe> {
