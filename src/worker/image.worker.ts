...
let currentTask: WorkerTask | undefined;
let enhancerOptions: ResolvedEnhancerOptions | undefined;
let predictor: ModelPredictor | undefined;
...
function ensureNotCancelled(task: WorkerTask, stage?: TaskStatus): void {
...
function validateDimensions(width: number, height: number): void {
  const options = enhancerOptions;
  if (options === undefined) throw new EnhancerError("NOT_INITIALIZED", "Worker is not initialized");
...
function timingsWithTotal(
  partial: Omit<StageTimings, "processingMs">,
  started: number
): StageTimings {
  return {
    ...partial,
...
async function initialize(options: ResolvedEnhancerOptions): Promise<EnhancerCapabilities> {
  ...
  const metadata = (await metadataResponse.json()) as { architecture?: string; input?: { width?: number; height?: number } };
...
async function runTask(task: WorkerTask): Promise<void> {
  ...
  try {
    ...
    const inputInfo: NonNullable<TaskInfo["input"]> = {
    ...
    let processingBackend: ProcessingBackend;
    let processed: OffscreenCanvas;
    const hooks = {
...
async function processQueue(): Promise<void> {
  if (currentTask !== undefined || disposed) return;
  const next = queue.shift();
  if (next === undefined) return;
  currentTask = next;
  await runTask(next);
  if (next.disposed) tasks.delete(next.taskId);
  currentTask = undefined;
  void processQueue();
}

function createTask(message: Extract<MainToWorkerMessage, { type: "CREATE_TASK" }>): void {
...
scope.addEventListener("message", (event: MessageEvent<MainToWorkerMessage>) => {
  ...
  if (message.type === "INIT") {
    ...
    if (task !== undefined) {
      task.disposed = true;
      abortTask(message.taskId);
      if (currentTask?.taskId !== message.taskId) tasks.delete(message.taskId);
    }
