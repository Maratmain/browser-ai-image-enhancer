...
import type { PreviewStatistics } from "./preprocess.js";

function clamp(value: number, minimum: number, maximum: number): number {
  return Math.min(maximum, Math.max(minimum, value));
}

function mix(neutral: number, predicted: number, strength: number): number {
...
export function applySafetyGuard(
  predicted: EnhancementParameters,
  statistics: PreviewStatistics
): EnhancementParameters {
  const pivot = clamp(statistics.medianLinearLuminance, 0.1, 0.5);
  const severity = Math.max(
    Math.abs(predicted.exposureEV) / 1.25,
    Math.abs(predicted.contrast - 1) / 0.45,
    Math.abs(predicted.saturation - 1) / 0.5
  );
