...
export interface PreviewStatistics {
  readonly medianLinearLuminance: number;
  readonly p05LinearLuminance: number;
  readonly p95LinearLuminance: number;
  readonly darkFraction: number;
  readonly brightFraction: number;
  readonly meanSaturation: number;
  readonly highlySaturatedFraction: number;
}

export interface PreparedModelInput {
  readonly values: Float32Array;
  readonly statistics: PreviewStatistics;
...
export function prepareModelInput(source: CanvasImageSource, inputSize = 64): PreparedModelInput {
  ...
  for (let index = 0; index < pixelCount; index += 1) {
    ...
    if (saturation > 0.85) highlySaturated += 1;
  }
